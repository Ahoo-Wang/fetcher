...
The view-engine package declares Node >=20.20.2 and is being rewritten. Its root entry stays independent of React and browser globals and is type-checked without the DOM library; hooks live in `/react` and browser controls in `/ui` with React 19. Engines are owned by the application or by `useViewEngine`, and open views are `ViewRuntime` instances that are disposed explicitly. Clipboard, page visibility and layout APIs remain browser capabilities. `MemoryViewStore` serves in-process examples and Node services; durable persistence is the application's `ViewStore` implementation against its own backend. Validate a framework-specific SSR/hydration integration instead of treating headless import support as certification of every UI component.
