...
`@ahoo-wang/fetcher-view-engine` declares direct dependencies on Wow and its UI implementation packages; these are separate from the peer-only arrows above. The package is being rewritten in the layers `model → filter → record | analysis | dashboard → runtime → react → ui`, with `store → model`, enforced by architecture tests. The root entry (models, pure kernels, runtime, the `ViewStore` port and `MemoryViewStore`) imports no React, DOM or CSS; `/react` provides headless hooks and `/ui` the shadcn/Base UI defaults. Durable persistence is the application's `ViewStore` implementation, and query sources remain application runtime adapters. See the [View Engine reference](../reference/view-engine/index.md) and the [architecture design](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/view-engine/docs/design.md).
