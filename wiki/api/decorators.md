...
Extends `ApiMetadata` without `method` and `path`:

| Property | Type | Description |
|----------|------|-------------|
| `basePath` | `string` | Endpoint-specific base path (overrides the class-level `basePath`) |
...
For HTTP methods not covered by convenience decorators (`HttpMethod` is exported by `@ahoo-wang/fetcher`, not by the decorator package):

```typescript
import { endpoint, autoGeneratedError } from '@ahoo-wang/fetcher-decorator';
...
4. **Inheritance (copy-on-write).** Parameter metadata resolves through the prototype chain, so subclasses inherit the base class's parameter bindings. When a subclass re-decorates a parameter of an inherited method, the inherited parameter map is copied before the subclass's own entries are added -- the base class's metadata is never mutated. ([`parameterDecorator.ts:217`](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/decorator/src/parameterDecorator.ts#L217))
...
**Source:** [`packages/decorator/src/parameterDecorator.ts:415`](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/decorator/src/parameterDecorator.ts#L415)
