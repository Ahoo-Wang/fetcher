...
The `@request()` decorator allows passing a `ParameterRequest` object for full control over the request. It is merged with endpoint-level configuration, with the parameter request taking precedence. ([`parameterDecorator.ts:379`](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/decorator/src/parameterDecorator.ts#L379))
...
The `@attribute()` decorator passes data to the exchange attributes, which can be read by any interceptor in the pipeline. ([`parameterDecorator.ts:415`](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/decorator/src/parameterDecorator.ts#L415))
...
Decorated endpoints are inheritance-aware:

- **Executor binding walks the prototype chain.** When `@api` decorates a class, it binds request executors for decorated methods found on the class's own prototype *and* on every ancestor prototype, so endpoints defined on a base class keep working on subclasses. ([`apiDecorator.ts:249`](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/decorator/src/apiDecorator.ts#L249))
- **Metadata reads resolve through the prototype chain.** Endpoint and parameter metadata are read with `Reflect.getMetadata`, so a subclass inherits the base class's bindings even when it does not re-declare them. ([`apiDecorator.ts:118`](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/decorator/src/apiDecorator.ts#L118))
- **Parameter metadata is copy-on-write.** When a subclass re-decorates a parameter of an inherited method, `parameter()` copies the inherited parameter `Map` before adding the subclass's own entries. The subclass sees inherited + own parameters, while the base class's metadata is never mutated. ([`parameterDecorator.ts:217`](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/decorator/src/parameterDecorator.ts#L217))
...
class AdminService extends BaseService {
  // Re-declares the inherited endpoint with an extra parameter.
  // AdminService resolves @path('id') + @query('audit');
  // BaseService metadata remains the single @path('id') parameter.
  @get('/users/{id}')
  getUser(
    @path('id') id: string,
...
| `endpoint` | Decorator factory | [`endpointDecorator.ts`](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/decorator/src/endpointDecorator.ts) |
...
| `parameter` | Decorator factory | [`parameterDecorator.ts`](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/decorator/src/parameterDecorator.ts) |
...
| `MethodEndpointMetadata` | Type | [`endpointDecorator.ts`](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/decorator/src/endpointDecorator.ts) |
...
| `ParameterRequest` | Interface | [`parameterDecorator.ts`](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/decorator/src/parameterDecorator.ts) |
...
| `AutoGenerated` | Class (Error) | [`generated.ts`](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/decorator/src/generated.ts) |
