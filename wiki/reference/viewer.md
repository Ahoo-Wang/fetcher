...
`Viewer` explicitly does not fetch rows or remotely persist views. `FetcherViewer` uses the default Fetcher and built-in viewer endpoints. [packages/viewer/src/viewer/Viewer.tsx:39](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/viewer/src/viewer/Viewer.tsx#L39) [packages/viewer/src/fetcherviewer/FetcherViewer.tsx:69](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/viewer/src/fetcherviewer/FetcherViewer.tsx#L69)
...
`FetcherViewer` keeps the last selected view ID in the module-scope `KeyStorage` named `fetcher-viewer-local-default-view-id`. Initial selection uses explicit `defaultViewId`, then the stored ID, the `isDefault` view, and the first view. Later user selection is retained when views reload; the selected ID is rebound to the returned view object. [packages/viewer/src/fetcherviewer/FetcherViewer.tsx:97](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/viewer/src/fetcherviewer/FetcherViewer.tsx#L97) [packages/viewer/src/fetcherviewer/FetcherViewer.tsx:587](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/viewer/src/fetcherviewer/FetcherViewer.tsx#L587)

Creation and update request the `PROCESSED` command stage and wait for a snapshot whose `version` reaches the returned `aggregateVersion`. Confirmation also matches the command result's tenant and aggregate identity and the expected owner. A newer snapshot from a different tenant or owner cannot confirm the save. The backend must expose the existing snapshot-list endpoint with `version` and `state`. If the command returns no valid target version, the component reports that the save cannot be confirmed and offers to reload views for browsing; that recovery neither resubmits the command nor invokes its success callback. Business failures are checked with `ErrorCodes.isSucceeded` before snapshot confirmation: the reported command error is shown and no success callback is invoked. While a save is pending, `refreshData` and matching refresh events do not reload a query belonging to the previous view. [packages/viewer/src/fetcherviewer/FetcherViewer.tsx:430](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/viewer/src/fetcherviewer/FetcherViewer.tsx#L430)

If an update command returns a business failure, `Viewer` stays mounted and preserves draft filters, sort, and columns, so the user can correct them and save again. A failed create still offers to reload views for browsing without resubmitting the command or invoking its success callback. [packages/viewer/src/fetcherviewer/FetcherViewer.tsx:211](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/viewer/src/fetcherviewer/FetcherViewer.tsx#L211) [packages/viewer/src/fetcherviewer/FetcherViewer.tsx:536](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/viewer/src/fetcherviewer/FetcherViewer.tsx#L536)
...
`FetcherViewer` loads the definition, saved views, then a paged POST query using `definition.dataUrl`; it combines a view's `internalCondition` with its visible condition, and accepts optional async `enhanceDataSource`. It exposes `refreshData`, `clearSelectedRowKeys`, `getPageQuery`, `getActiveView`, and `getViewerDefinition` through its ref. [packages/viewer/src/fetcherviewer/hooks/useFetchData.ts:47](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/viewer/src/fetcherviewer/hooks/useFetchData.ts#L47) [packages/viewer/src/fetcherviewer/FetcherViewer.tsx:61](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/viewer/src/fetcherviewer/FetcherViewer.tsx#L61)
...
| View or query changed | Previous rows stay hidden after changing the view, page, filters or sort until results belong to the current request. Refreshing the same query may retain its existing result. [packages/viewer/src/fetcherviewer/hooks/useFetchData.ts:144](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/viewer/src/fetcherviewer/hooks/useFetchData.ts#L144) |
| Definition error | Renders the built-in definition-error UI. [packages/viewer/src/fetcherviewer/FetcherViewer.tsx:493](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/viewer/src/fetcherviewer/FetcherViewer.tsx#L493) |
| Missing definition (empty result) | Renders `未找到视图定义`. [packages/viewer/src/fetcherviewer/FetcherViewer.tsx:501](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/viewer/src/fetcherviewer/FetcherViewer.tsx#L501) |
| No saved views (empty result) | Renders `未找到视图` with a `创建视图` action, including after deleting the last view. [packages/viewer/src/fetcherviewer/FetcherViewer.tsx:507](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/viewer/src/fetcherviewer/FetcherViewer.tsx#L507) |
| Views error | After a successful create/update, failed refreshes, missing views, and snapshot versions behind the command result keep the retry action available. Retrying reloads snapshots without repeating the command. Other initial views errors are not rendered separately. [packages/viewer/src/fetcherviewer/FetcherViewer.tsx:430](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/viewer/src/fetcherviewer/FetcherViewer.tsx#L430) |
| Data request error | The data hook returns an error; `FetcherViewer` does not render a dedicated data-request error UI. [packages/viewer/src/fetcherviewer/FetcherViewer.tsx:224](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/viewer/src/fetcherviewer/FetcherViewer.tsx#L224) |
| `enhanceDataSource` rejection | The effect catches the rejection and renders `处理视图数据失败` in an alert. The viewer stays mounted, preserving edited filters, sort, columns and pagination. A successful enhancement clears the error; late results from a replaced effect are ignored. [packages/viewer/src/fetcherviewer/FetcherViewer.tsx:259](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/viewer/src/fetcherviewer/FetcherViewer.tsx#L259) [packages/viewer/src/fetcherviewer/FetcherViewer.tsx:542](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/viewer/src/fetcherviewer/FetcherViewer.tsx#L542) |

Configure the default Fetcher before mounting. `ownerId` and `tenantId` default to `'(0)'`; changing the viewer definition, tenant, or owner remounts the content and reloads that scope. [packages/viewer/src/fetcherviewer/FetcherViewer.tsx:102](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/viewer/src/fetcherviewer/FetcherViewer.tsx#L102) [packages/viewer/src/fetcherviewer/FetcherViewer.tsx:118](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/viewer/src/fetcherviewer/FetcherViewer.tsx#L118)
...
| Need custom views/data error UI | Compose public `useViewerDefinition`, `useViewerViews`, and `useFetchData` with `Viewer` when custom initial-view or data-request errors and retry controls are needed. `FetcherViewer` already renders enhancement failures. |
