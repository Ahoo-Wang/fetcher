...
安装时还需要解析该包声明的全部 peer 依赖；完整清单见[包接入前提](../../reference/cosec/index.md)。以下服务路由和身份由应用提供，不由安装过程创建。

## 1. 确认认证契约

安装 `@ahoo-wang/fetcher` 和 `@ahoo-wang/fetcher-cosec`。应用须已有登录流程，返回 JWT 字符串 `accessToken` 和 `refreshToken`。本例的 `POST /auth/refresh` 接收该令牌对的 JSON 并返回新令牌对，`GET /profile` 返回 `{ id, name }`。这些路由和 app ID 必须与后端一致。仅对该可信来源使用此客户端。下面的 `isTrusted: sameOriginTrust` 让令牌和设备 ID 只发往 `baseURL` 所在源与页面自身的源；不设它时，发往其他任意源绝对 URL 的请求（分页链接、下载地址）会收到访问令牌。
...
在测试中使用内存令牌/设备存储和模拟 fetch，覆盖有效令牌、访问令牌过期但刷新令牌有效、并发请求、刷新失败及 401/403 响应。刷新并发在单个 token manager 内共享，不是跨标签页的分布式锁。刷新因另一个标签页已用掉一次性刷新令牌而失败时，本标签页会重新读取存储，并用那个标签页存入的令牌继续；其他刷新失败可能清除当前会话；回调不会将失败请求变成成功结果。除了显示登录或错误界面，也要捕获请求拒绝。

## 5. 清理所有者

当前 `TokenStorage` 默认使用按存储 key 命名的广播总线；其他上下文需要匹配通道和支持的 messenger。这与默认只使用本地总线的普通 `KeyStorage` 不同。广播是变更通知，不是跨标签页的事务性刷新协调。

应用关闭时停止尚未完成的应用任务，并调用 `session.dispose()`。每个存储的 `destroy()` 释放自己的处理器并关闭自己创建的广播总线，不删除令牌；退出登录需另外调用 `signOut()`。 存储通知是异步的：退出登录后通知仍在投递时，不要立即销毁会话。投递中关闭总线可能中断广播并记录错误。不要销毁其他所有者仍在共享的总线。配置器没有 dispose 方法；本例随会话一起停止使用私有 Fetcher。
