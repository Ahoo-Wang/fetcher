...
| `headers?: RequestHeaders` | `{'Content-Type': 'application/json'}` | 自定义对象替换构造默认头，客户端保存自己的副本；每个请求防御性复制并按大小写不敏感规则合并。 |
...
`new NamedFetcher(name, options?)` 继承 `Fetcher`，并立即注册到单例 `fetcherRegistrar`；它保存在 `globalThis` 上，同一进程中的第二份包副本（ESM 与 CommonJS 构建，或两个版本）共享它。导出的 `fetcher` 是默认命名实例，`DEFAULT_FETCHER_NAME = 'default'`；第二份副本复用已注册的默认实例，而不替换它。同名注册替换原有客户端。
...
`getFetcher(fetcher?, defaultFetcher?)` 接受实例、注册名或空值。字符串视为名称，必须已注册；其他值直接作为 fetcher 使用（不做 `instanceof` 检查，另一份包副本创建的 fetcher 也可用）；假值优先选择传入的后备客户端，再选择全局默认实例。`FetcherCapable` 声明此可选字段，`NamedCapable` 声明 `name`，`FetcherConfigurer.applyTo(fetcher): void` 是扩展配置器使用的结构契约。
