...
终止标记在 JSON 解析前识别，不作为 ChatResponse 发出。其他无效 JSON 会使流消费失败。在 `data: [DONE]` 之前结束的流（连接中断或服务端提前停止）会在迭代时以 `@ahoo-wang/fetcher-eventstream` 的 `EventStreamIncompleteError` 失败，不会把不完整的回答当成完整回答。请求 Promise 成功只表示拿到了流，读取时仍可能失败。提取器不拼接 choices、不执行工具调用、不估计 usage，也不重连。

## 完整流式函数

`chat.completions(request, signal)` 接受单次调用 signal。不使用 ChatClient 时，按下例使用底层 Fetcher 请求 API 并指定 CompletionStreamResultExtractor。AbortController 可终止未完成的请求；提前结束时取消已获得的 reader，并始终释放锁。
