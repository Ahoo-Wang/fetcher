...
`OpenAI` 始终创建自己的 Fetcher 和 Authorization 头。复用已有认证代理时，应构造 `ChatClient({ fetcher })`，这样也会保留该 Fetcher 的请求头、超时和拦截器。ApiMetadata 应通过构造器传入：装饰器执行器按方法缓存元数据，只在 `apiMetadata` 对象被替换时重建，原地修改不会生效。
