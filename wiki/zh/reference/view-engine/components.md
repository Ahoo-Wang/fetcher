...
切换到数据表时保留图表轴、系列、指标选择和显示偏好，切回后恢复。编辑或删除查询输出会保留展示引用；新查询成功前，旧结果始终使用执行时的 schema。失效映射仅由用户明确选择或修复操作调整，有歧义的初始映射保持未选择。柱状图与面积图将正负 SUM 值分别堆叠在零轴两侧；null 或缺失组合提示不兼容，数据表仍可查看。 仅切换布局不会截断有效指标选择，也不会替换用户主动清空的选择。多指标切换到饼图时保留选择并提示兼容性问题，直到用户明确选定一个支持的指标；直接切回原布局时保留原有选择。 可选默认值在渲染时解析，不反写进草稿，因此单纯往返切换不会把原本未修改的视图标记为待保存。

`projectAnalysis(plan, rows, presentation)` 接收已校验行，返回 `{ plan, issues, issueSummary?, points, series, x?, continuous }`。投影 plan 应用列顺序/宽度并追加未指定列。维度、单位或指标语义不兼容时，返回可操作的错误说明并回退表格。指标卡要求未分组结果；折线/面积图要求连续数值或时间轴；饼图要求 SUM 或 COUNT、无 null/负值且没有系列拆分。图表最多 500 个已返回分组、12 个系列、24 个饼图扇区。投影不会重新计算平均值、补造零桶，也不会把截断分组宣称为全量合计。重复显示标签不会合并带类型的组合身份。分析表格使用固定列布局，默认列宽 160px（可由列宽配置覆盖），窄容器横向滚动；文本截断时通过原生 title 保留完整单元格值。`AnalysisTable.sortDisabled` 默认 false；工作输入不能运行时将其设为 true，不把已显示结果误标为加载中。
...
`AnalysisPresentationEditor.chartOnly` 默认 false；完整分析视图开启此选项，数据表由底部模式切换负责，未选择图表时不显示其字段设置。

查询 Sheet 使用“运行并查看”：`engine.analysis(id).start()` 同步校验并捕获查询，返回 `{ accepted, completion }`。校验可能抛错；拒绝执行返回 `accepted: false`。仅在接受执行时关闭 Sheet，独立处理 `completion`。`run()` 等待同一执行流程完成。`AnalysisSession.queryAttempt` 保留最近接纳的计划，包括失败后；`pendingQuery` 仅描述当前请求。切回失败实例不会隐式重试。

结果摘要始终使用最近成功执行的配置、schema、行数据与本地接收时间。达到已知上限只说明可能仍有其他分组，不代表确认截断或全局总量。未运行草稿与失败反馈和旧结果同时保留；工作查询不一致时禁止表头排序。保存与运行保持独立。切换实例保留原始编辑输入、查看模式和面板偏好。数据表模式隐藏可视化面板但不重置它；明确点击可视化配置才切换分析并展开。现有图表类型按成功结果检查兼容性，仅初始化唯一字段映射，样式按需展开。这些展示操作均不查询。

展示方式使用五张图标与名称卡片：指标卡、柱状图、折线图、面积图、饼图。所有类型始终显示，不可用项说明原因。可用表示最近成功结果的 schema 与行数据存在至少一个合法映射，不要求默认映射恰好可用。纯能力推断通过已有投影校验器检查所有受支持的轴与单指标候选；增加指标不能修复已失败的单指标映射。推荐采用确定规则：无分组指标，其次时间趋势，其次分类比较，不自动选择类型。选定后在卡片下方展示数据映射，再提供默认折叠的样式设置；不增加向导或业务阶段状态。有歧义的映射保持未选择。已保存类型或选中指标变得不兼容时仍显示并保留，直到用户明确修复。未启用 `chartOnly` 的独立编辑器保留包含数据表的通用展示选择器；分析工作台卡片不包含数据表，其核对入口仍在底部双模式。

编辑器和投影器在筛选可选候选之前使用一致的隐式指标选择语义；当前不兼容指标仍然可见，包括隐式选中的指标。投影失败可提供 `issueSummary` 作为卡片短原因，完整原因保留在可展开说明中。映射错误按场景指出选中数量与不支持的聚合方式。中央结果提示保持简短；“修复配置”展开保留的编辑器，并聚焦第一个无效且可操作的控件，没有具体字段时聚焦数据映射标题。该操作不改变查询或展示配置。

维度拥有的显示输出参与高级服务端排序选择。新显示字段的 alias 避开所有维度、指标与已有显示输出；切换已有显示字段时保留 alias。完整查询配置无效时暂停自动刷新。分析结果被淘汰后，图表空态在当前查询合法时提供明确的重新运行入口，不改变自动加载策略。执行摘要将无条件元素筛选描述为全部记录。

## 扩展指标编辑器

宿主启用后，分析查询编辑器提供去重计数、P50/P95/P99 与自定义百分位、指标独立筛选、派生公式和结果筛选。派生公式按业务标题选择前置指标，并持久化稳定 ID；派生值使用明确的数字/百分比展示选项。维度设置提供缺失值归组及单维日期缺口补齐。

控件保留未完成草稿、遵守禁用状态，并保留失效引用以便修复。明细作用域内的指标统计/筛选明细记录，不能解释为根订单数。能力、限制和统计语义参见[模型](./models.md#扩展聚合分析)。

`AnalysisEditorProps.filterEditors` 将定义级筛选编辑器映射传给指标与元素筛选。指标筛选撤销以同一作用域内、同 ID 指标最近执行的配置为基线，而非首次挂载值。执行口径包含已保留查询计划中的分桶参数（缺失值归桶、数值间隔、日期单位/时区及空桶补齐）、各指标谓词、百分位参数、统计表达式、派生公式和 HAVING。

共享坐标轴要求指标单位和数值格式一致；百分比与普通数字不能混用。指标卡和表格仍可分别格式化各指标。
