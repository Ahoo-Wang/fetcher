...
React Hook 把状态保存在挂载的组件内部，不在组件或请求之间缓存响应。在服务端渲染时，Hook 仍使用传给它的客户端与存储，因此上面的身份规则同样适用于你传入的客户端。`useKeyStorage`（以及基于它的 `useSecurity` 与 `SecurityProvider`）在服务端不读取存储：服务端与水合期间渲染默认值，随后渲染存储值，因此水合结果与服务端 HTML 一致。租户或用户变化时重新挂载子树，可以分隔 UI 状态；服务端仍须实施访问控制。具体 SSR 框架仍需要导入、渲染和水合验证，本章不代表所有框架已通过认证。
