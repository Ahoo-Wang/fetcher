...
继承 `ApiMetadata`，不包含 `method` 和 `path`：

| 属性 | 类型 | 描述 |
|----------|------|-------------|
| `basePath` | `string` | 端点级别的基础路径（覆盖类级别的 `basePath`） |
...
用于便捷装饰器未覆盖的 HTTP 方法（`HttpMethod` 由 `@ahoo-wang/fetcher` 导出，而非装饰器包）：
...
4. **继承（写时复制）。** 参数元数据沿原型链解析，因此子类会继承基类的参数绑定。当子类重新装饰继承方法的参数时，会先复制继承的参数映射，再添加子类自己的条目 -- 基类的元数据永远不会被修改。([`parameterDecorator.ts:217`](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/decorator/src/parameterDecorator.ts#L217))
...
**源码:** [`packages/decorator/src/parameterDecorator.ts:415`](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/decorator/src/parameterDecorator.ts#L415)
