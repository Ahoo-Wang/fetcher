...
`@request()` 装饰器允许传入 `ParameterRequest` 对象以完全控制请求。它会与端点级别的配置合并，参数请求具有更高优先级。([`parameterDecorator.ts:379`](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/decorator/src/parameterDecorator.ts#L379))
...
`@attribute()` 装饰器将数据传递到 exchange 属性中，可被管道中的任何拦截器读取。([`parameterDecorator.ts:415`](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/decorator/src/parameterDecorator.ts#L415))
...
- **执行器绑定会遍历原型链。** 当 `@api` 装饰一个类时，它会为类自身原型以及所有祖先原型上找到的被装饰方法绑定请求执行器，因此定义在基类上的端点在子类中依然可用。([`apiDecorator.ts:249`](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/decorator/src/apiDecorator.ts#L249))
- **元数据读取沿原型链解析。** 端点和参数元数据通过 `Reflect.getMetadata` 读取，因此即使子类没有重新声明，也会继承基类的绑定。([`apiDecorator.ts:118`](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/decorator/src/apiDecorator.ts#L118))
- **参数元数据采用写时复制（copy-on-write）。** 当子类重新装饰继承方法的参数时，`parameter()` 会先复制继承来的参数 `Map`，再添加子类自己的条目。子类可以看到继承的 + 自己的参数，而基类的元数据永远不会被修改。([`parameterDecorator.ts:217`](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/decorator/src/parameterDecorator.ts#L217))
...
class AdminService extends BaseService {
  // 重新声明继承的端点并添加额外参数。
  // AdminService 解析到 @path('id') + @query('audit')；
  // BaseService 的元数据仍只有 @path('id') 一个参数。
  @get('/users/{id}')
  getUser(
    @path('id') id: string,
...
| `endpoint` | 装饰器工厂 | [`endpointDecorator.ts`](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/decorator/src/endpointDecorator.ts) |
...
| `parameter` | 装饰器工厂 | [`parameterDecorator.ts`](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/decorator/src/parameterDecorator.ts) |
...
| `MethodEndpointMetadata` | 类型 | [`endpointDecorator.ts`](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/decorator/src/endpointDecorator.ts) |
...
| `ParameterRequest` | 接口 | [`parameterDecorator.ts`](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/decorator/src/parameterDecorator.ts) |
...
| `AutoGenerated` | 类（Error） | [`generated.ts`](https://github.com/Ahoo-Wang/fetcher/blob/main/packages/decorator/src/generated.ts) |
