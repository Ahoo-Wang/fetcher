...
export class ChatClient implements ApiMetadataCapable, ExecuteLifeCycle {
   ...
   *                       the `fetcher` (which carries the base URL and interceptors),
   *                       `headers`, `timeout` and the other ApiMetadata fields.
   *                       If not provided, the default fetcher is used.
   *
   * @example
   * ```typescript
   ...
   * const client = new ChatClient();
   *
   * // With a fetcher for the API endpoint, plus extra headers and a timeout
   * const client = new ChatClient({
   *   fetcher: new Fetcher({ baseURL: 'https://api.openai.com/v1' }),
   *   headers: { Authorization: `Bearer ${process.env.OPENAI_API_KEY}` },
   *   timeout: 30000,
   * });
   * ```
   ...
   * @param signal - Optional signal that cancels the request (and, for a
   *                 stream, the connection) when it aborts.
   * @returns Promise resolving to:
   *          - `JsonServerSentEventStream<ChatResponse>` when `T['stream'] extends true`
   *          - `ChatResponse` when streaming is disabled or omitted; a dynamic stream flag returns a union
