...
it('uses a new serializer when storage takes over an idle automatic transformer', async () => {
  const key = randomUUID();
  const backing = new InMemoryStorage();
  const bus = new BroadcastTypedEventBus<StorageEvent<Counter>>({
    delegate: new SerialTypedEventBus(key),
  ...
  first.destroy();
  const nextSerializer: Serializer<string, Counter> = {
    serialize: value => `v2:${value.read()}`,
    deserialize: raw => {
      if (!raw.startsWith('v2:')) throw new Error('Expected v2 counter');
      return new Counter(Number(raw.slice(3)));
    },
  };
  backing.setItem(key, 'v2:1');
  const receiver = new KeyStorage({
    key,
    storage: backing,
    eventBus: bus,
...
it('delivers a valid new value when the stored old snapshot cannot be decoded', async () => {
  const key = randomUUID();
  const backing = new InMemoryStorage();
  backing.setItem(key, '1');
