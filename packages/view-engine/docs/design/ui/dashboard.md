...
- **地址是宿主的**：`DashboardWorkbench` 的 `initialFilters`／`onFiltersChange`（批 C1）；浏览器里「ValuesReachTheHost」。嵌入的板子是 `filterValues`／`onFiltersChange`，每个筛选另有页面给的三态（可编辑、锁定、隐藏），见 [embed.md](embed.md#仪表盘的筛选逐个三态)。
