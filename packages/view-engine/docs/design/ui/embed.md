...
`interaction`，缺省 `read-only`——业务页面摆出的是别人定好的东西，更多交互由宿主明说：
...
- **锁定**：在筛选条上读作它的值——「客户 是 晨光食品」，与已应用条同一套读法（`describeFilter` + `summaryText`）——旁边一把锁（`IconTooltip`「由页面设定」），没有控件、没有 ✕；读屏念「客户（由页面设定）」。时间粒度锁定时读作「时间粒度 按月」。
- **隐藏**：不在筛选条上，照样收窄接上的面板；面板头上的「不受…影响」不提它（那是读者看不见的筛选）。
- 锁定与隐藏都是**页面的**：由 runtime 持有（`DashboardRuntime.holdFilters`，见 [../runtime.md#dashboard](../runtime.md#dashboard)），读者的每一条命令都碰不到——筛选条上的值、「清空」、按粒度、设它的交叉筛选点击（那块面板的点击退回追问菜单，面板头不再说「点击筛选」）。筛选条上只有锁定的筛选时没有「清空」。别的文本筛选，候选值在页面持有的值下计数：锁定在一位客户上的页面，别的筛选只列这位客户的数据里有的值。（见 test/embeddedDashboard.test.tsx「draws a locked filter as what it holds, keeps a hidden one off the bar, and lets the reader change neither」，test/embedRuntime.test.ts「sets aside a click that sets a filter the page holds: a press does what a panel without one does」「counts what a text filter offers under what the page holds」）
- **值是宿主的地址**：`filterValues` 进、`onFiltersChange` 出（打开那一刻也说一声），`initialTab`／`onTabChange` 同理。锁定与隐藏的筛选取 `filterValues` 里的值（没写就是它的默认值），**跟着这个属性变**——客户页换到下一位客户，板子跟着换，内容一样的新对象什么也不动；它们从第一次查询起就在（`OpenOptions.held`），不会有一次没锁的查询出去。读者的筛选在打开时读一次，与 `DashboardWorkbench` 的 `initialFilters` 同一个读法：点名了其中任何一个，它就是它们的全部；一个也没点名，就都从默认值开始。页面的值被板子拒绝（没有这个筛选、值读不懂）时，板子上方一条与「收窄被拒」同一句的 destructive `Alert`，能收的照收。（见 test/embeddedDashboard.test.tsx「follows the host address as it changes, and tells it what the filters hold」「opens the reader’s filters at their defaults when the page names only what it holds」「says what the board refuses of the page’s values, as a refused narrowing」，test/embedRuntime.test.ts「is in force from the first query when the board opens under it」「follows the page, puts a default back for null, and lets go」「answers what the board refuses of the page, every time it is asked, and takes the rest」）
