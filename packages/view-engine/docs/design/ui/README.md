...
- **key 是类型**：`MessageKey = keyof typeof en`，`MessageFormatters.label(key, params?, fallback?)` 只收它，所以删掉一个键或拼错一个键是编译错误而不是屏幕上的一行 code。由运行时值拼出的 key 走收紧后的源类型（`ViewKind`、`ViewAudience`、`RecordLayout`、`DateShape` 都是闭合联合，模板字面量因此可判）。唯一的例外是 `label.operator.*`：目录故意只命名读起来不成话的那几个，其余回退到组件自己派生的拼写，所以 `messages.ts` 里用一个 `operatorKey()` 把这处断言圈起来，全包仅此一处；
