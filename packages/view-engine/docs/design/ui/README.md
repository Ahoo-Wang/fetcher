...
- **焦点是同一个 token 的两种画法**。vendored `Button` 以 1px `border-ring` 加 3px `ring-ring/50` 光晕表示焦点，指示的是那 1px 边、光晕只是强调；没有边可染的裸按钮（表头排序、已应用条的 ✕）用 `ui/layout.ts` 的 `FOCUS_RING`——同一个 `--ring` 的 2px `outline`。`--ring` 与 `--input` 一样钉在两个主题都 ≥3:1（WCAG 1.4.11 对焦点指示的要求），由浏览器故事 `FocusIndicatorsInLightTheme`／`InDarkTheme` 聚焦三种控件量层叠色；暗色的 `--border` 提到 20% 白，行线在暗色卡片上 ≥1.5:1（`DarkHairlines`）。分隔线不是控件，不欠 3:1，但看不见的行线就是没有行线。
