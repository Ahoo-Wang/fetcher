...
- **可选档位按 `runtime.limits` 裁剪**（`minRefreshInterval`／`maxRefreshInterval`），限制不允许的档位**不存在**而不是禁用（D4）；正在跑的那个若在限制之内就并进梯子里（与每页条数同一条读法），若被限制拒绝则不并——那份配置本就被准入拒绝、状态条已经在说，菜单要给的是出路（关闭，或一个能用的档位）而不是那个坏掉的数。什么都不能选、也没有在跑的间隔时，连 `▾` 都不出现；
