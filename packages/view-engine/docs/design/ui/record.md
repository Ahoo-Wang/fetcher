...
- **主键的"固定在左侧"是投影说了算，不是面板说了算**：`projectRecord` 给主键列一律发 `pinned: 'left'`，无论配置写了什么，`RecordTable` 因此真的把它冻住。这是定义的性质而不是偏好——那一列说明这一行是哪条记录，所以它是横向滚动时唯一必须留在视野里的列——所以不写进配置，也不让用户改。只在面板里画成"固定"而不落到读它的地方，等于面板许诺了一件表格不会做的事；
