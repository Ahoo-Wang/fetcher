...
- **三项**（`ui/analysis/DrillMenu.tsx`，画的是 `useAnalysisResult().followUp(row)` 交出的动作列表，哪几项、什么顺序、按下去做什么都由控制器定）：**查看这些记录**（`workbench.drill(conditions, title)`，在同一个工作台里开出未保存的记录视图，带「返回」那一条，见 [react.md](../react.md) 的「持有的视图」；`canDrill` 为假时这一项不在——不是禁用，是不画）、**再按…拆一层**（一层子菜单，列出能分组、当前结果又还没按它分的字段；选中即 `splitBy` 后 `apply`）、**只看这一组**（`workbench.follow(config, title, conditions)`：跑过的那份配置加上 `focusOn`，布局与图照屏幕上的草稿，开成**旁边一个未保存的分析视图**，同样带「返回」）。拆一层改的是当前这个分析视图的配置，所以它和手改编辑器一样会变脏、可撤、可保存；
- **只看这一组是旁边一个新问题，不是改写这一个**（2026-09-23 审查）。它从前把这一组写进当前视图的范围并应用：没有返回，回去只能手动删条件、再跑一遍，而且一个保存过的视图就此变脏，一次「保存」就把它存成了只看华南的那个。现在它与「查看这些记录」走同一条路（`WorkbenchController.follow`，与 `drill` 共用持有与返回）：原来那个视图连同它的结果在底下开着、不变脏，「返回」把那次结果原样放回来、不重跑；新开的那个第一次保存是另存，和任何新视图一样。从它还能再追问——再查看记录、再只看一组——返回一层一层退回去（`held.from`）；
- **从一组开出来的视图按它是什么起名**：`「{是什么} · {这一组}」`（`label.drill.titled`）。记录的「是什么」是定义的标题（`runtime.definition.title`，「订单」）——它是订单里华南那一组，不是一个分析，所以不叫来源分析的名字；只看这一组的「是什么」是来源视图的名字——同一个问题，只问这一组。「这一组」就是菜单标题那句话。从前两者都叫「新视图」（`newView.title`），读不出它是什么，保存时也只能从「新视图」改起。标题是文案，`/react` 不带文案，所以由菜单说出来、经 `run(title)` 交给工作台；
