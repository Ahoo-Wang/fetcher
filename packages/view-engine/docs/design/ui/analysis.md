...
- **读起来像一张分析师的表，配方全是记录视图的**（2026-09-23 审查 P1）：
  ...
  - **点表头排序**：表头就是记录视图的 `SortableHeader`（`tableColumns.ts` 的 `headerColumnOf` 把分析列交成它要的 `RecordColumnView`，别名当字段）——同一颗按钮、同一套箭头与优先级数字、`aria-sort` 只挂在主排序列上、整行表头一个 Tab 停靠点（←／→ 在列间走）、Shift／Ctrl／⌘ 追加。按下的循环是**升序 → 降序 → 回到视图原来的次序**（`ui/analysis/headerSort.ts` 的 `headerSorted`）：记录视图第三下是把这列拿掉，那就是视图的次序；分析不是——排序决定「前 N 组」**留哪几组**，所以第三下要回到按表头之前的那个次序（`base`：开始按表头时生效的排序；只要生效的排序还是表头写的那个就一直记着，从托盘、保存的视图或放弃改动来的排序重新起算）。视图原本就是这一列降序时，降序就是回去的路，这一列在两个方向之间切换。写法是 `setSort` + `submit`（`useHeaderSort`），与记录视图表头的 edit + apply、合计行开关的 `setTotals` + `submit` 同理：排序是问题的一员，托盘里改它会「改了就跑」，但范围还等着应用或关了自动运行时不跑，而对着结果按下表头、屏幕上什么也没变，读起来就是表头坏了。下一个排序按**跑过的那份配置**的排序算（箭头是用户看见的那个），`aria-sort` 同样按它。只在工作台里（`AnalysisParts`）、且跑过的配置有维度时才可按——Wow 拒绝对无分组聚合排序；仪表盘面板与嵌入视图的表头只是列名（仍是 `SortableHeader`，不可排序时格子自己当那个停靠点，被截短的列名可以走到、读到）。
    （见 test/analysisTable.test.tsx「an analysis number reads from the right」「an identifier dimension」「an analysis column keeps its width」「an analysis header that sorts」、test/analysisTableSort.test.tsx 与浏览器故事「分析工作台/回归」的 `TableReadsLikeATable`、`ColumnsHoldStill`、`HeaderSorts`、`IdentifiersInMonospace`）
