...
- **固定列还没有真的固定**——为什么：列设置已经能把列钉在左右两侧、写进 `table.columns[].pinned`，内核也把它投影进 `RecordColumnView`，但 `RecordTable` 只有操作列是 sticky，别的列的 `pinned` 画不出来——一个存得下、读得回、看不见的设置。判据：`RecordTable` 按 `column.pinned` 给左右两侧的列加 sticky 定位与偏移（左侧按前面各列宽度累加，右侧同理），横向滚动时固定列不动，汇总行与表头跟着一起固定；`test/recordTable.test.tsx` 覆盖两侧各一列。落点：`src/ui/RecordTable.tsx`、[ui/record.md#columnsettings列的顺序显隐固定与汇总](ui/record.md#columnsettings列的顺序显隐固定与汇总)。
