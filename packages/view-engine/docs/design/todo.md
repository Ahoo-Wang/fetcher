...
- **铺满屏幕时结果要接住多出来的高度**——为什么：视图铺满后确实占满了视口（`data-view-expanded` + `position: fixed; inset: 0`），但结果块只长到内容那么高，`RecordTable` 的滚动容器仍是 `max-h: var(--fve-record-table-max-h, 70vh)`——`max-height` 只封顶不拉伸，所以行数不多时下半屏是空的：铺满换来的是更多的**列**而不是更多的**行**，而这一条的理由本来就是「答案得到多大地方」（[ui/README.md#工作台骨架](ui/README.md#工作台骨架)、[decisions.md#d10-铺满屏幕是就地展开而且不是模态](decisions.md#d10-铺满屏幕是就地展开而且不是模态)）。判据：铺满时主列成为一条定高的弹性列（整条祖先链带上 `min-h-0`，外壳自己 `overflow: hidden` 而不是 `auto`），结果块 `flex-1`，表格的滚动容器改吃可用高度而不是 `70vh`；未铺满时的布局与 `scrolls={false}`（仪表盘面板顶着外层滚）两条路径都不受影响；`test/viewExpansion.test.tsx` 与回归 story `FillTheScreen` 各补一条按实际几何量的断言——表头仍粘顶、汇总仍贴底、冻结列仍不动，这三样是这次改动最容易碰坏的。落点：`src/ui/WorkbenchShell.tsx`、`src/styles.css`、[ui/record.md#表格-chrome层次与冻结列](ui/record.md#表格-chrome层次与冻结列)。
