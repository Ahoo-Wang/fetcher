...
- **Q7 切换视图要不要保持展开**：展开（填满屏幕）现在随视图切换而结束，因为旧 runtime 释放时 `expandable && open` 掉了一下，这与编辑带"折叠属于这一次打开"的规则一致。另一种读法是展开更像全屏——它是工作区级的姿态，不是某个视图的状态，切视图该保持。两种行为都有测试记着（[ui/README.md](ui/README.md)）。
- **Q8 没有行操作时，右边还要不要有冻结列**：规则是"首列、尾列必定是固定列"。左框是主键（投影恒定左钉），右框是行操作；没有行操作时右边是否仍要钉住最后一列尚未定。这个结论卡着表格固定列视觉那条 TODO（[ui/record.md](ui/record.md)）。
