...
- **日期**：2026-09-19
- **决定**：视图铺满屏幕由 `useViewExpansion` 在 `ViewSurface` 的根上写一个 `data-view-expanded`，一条不进 layer 的 CSS 把它钉在视口上——**不 portal、不重新挂载**。它**不声明模态**：没有 `aria-modal`、不困住焦点、其余部分不设 inert；只借走"背景不滚"这一样，按 `document` 计数、连 `!important` 一起原样还回去。Esc 收起，但对话框／菜单／列表框（含焦点仍在 `data-popup-open` 触发器上的）先于它拿到这个键，收起后焦点回到开它的控件。状态归 `WorkbenchShell`，与侧栏折叠同一类：不入库、不记忆、不经离开守卫。`EmbeddedView` 不长自己的开关，只把 `ref` 与这个 hook 交给宿主。
- **依据**：重新挂载会把用户正看着的草稿连同选择与开着的弹层一起丢掉，而那份草稿只活在这一次打开里；声称模态是一个关于内容的承诺——这块内容还是宿主 DOM 的后代，页面其余部分并没有不可用——把"其余部分"设成 inert 则要一路向上在每层给兄弟节点加 inert，那正是 portal 的活。嵌入视图是结果本身、别无他物，为一个按钮给它造一行 chrome，就是在宿主页面上摆一件它没要的东西。
- **落点**：[ui/README.md#工作台骨架](ui/README.md#工作台骨架)
