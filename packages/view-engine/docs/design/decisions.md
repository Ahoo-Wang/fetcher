...
- **Q22 读者筛选的初值没点名的算什么**：现状与 `DashboardWorkbench` 的 `initialFilters` 同一读法——`initialFilters` 点名了任何一个读者的筛选，它就是它们的全部（没点名的为空）；一个也没点名才从默认值开始。Metabase 是「没点名的取默认值、地址里显式写空」。推荐保留现状，两个入口一种读法；要改就两个一起改，地址里的「空」要有写法。
