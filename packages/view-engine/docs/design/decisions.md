...
- **Q19 锁定的筛选画不画出来**（阶段 4，2026-09-23 提出）：现状是锁定的筛选**在筛选条上读作它的值**（「客户 是 晨光食品」、带锁、没有控件），隐藏的才不画——本包「说清作用范围」的原则，与 `EmbeddedView` 的 `scopeFilter` 在已应用条上单成一组同理。Metabase 静态嵌入的「锁定」参数不显示。推荐保留现状：不想让读者看见的，宿主用 `hidden`。
- **Q20 隐藏的筛选能不能被交叉筛选设**：现状是隐藏与锁定一样由页面持有，点一组不设它（那块面板退回追问菜单）——没有筛选条上的一枚，读者看不见板子为什么变了。Metabase SDK 的 `hiddenParameters` 只藏控件、值照样能被点击改。推荐保留现状。
- **Q21 两个嵌入缺省哪一档**：现状两者都缺省 `read-only`（业务页面摆出的是别人定好的；更多交互由宿主明说）。Metabase 没有缺省——宿主选组件（`StaticDashboard`／`InteractiveDashboard`／`EditableDashboard`）。推荐保留现状。
- **Q22 读者筛选的初值没点名的算什么**：现状与 `DashboardWorkbench` 的 `initialFilters` 同一读法——`filterValues` 点名了任何一个读者的筛选，它就是它们的全部（没点名的为空）；一个也没点名才从默认值开始。Metabase 是「没点名的取默认值、地址里显式写空」。推荐保留现状，两个入口一种读法；要改就两个一起改，地址里的「空」要有写法。
- **Q23 可编辑一档遇到存不了的板**：现状是「编辑」只给能保存这块板的人，系统板或没有保存权限时读作可交互一档，不给「另存为」——嵌入没有视图列表，另存出来的副本无处落脚。推荐保留现状。
- **Q24 记录嵌入的导出与搜索不看档位**：现状是 `withExport`／`withSearch` 是开关，只读一档也能开（Metabase 静态嵌入也可以开下载）。推荐保留现状。
- **Q25 `DashboardNavigation` 的名字**：它现在也是 `EmbeddedView` 的 `onNavigate` 的类型（在工作台中打开、追问）。推荐下一次重构时改名 `ViewNavigation`，本包没有兼容义务。
