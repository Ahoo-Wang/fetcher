...
- **宿主交来的未保存视图**（`unsaved: { title, config } | null`，D22 H，`react/workbench/unsavedView.ts`）：仪表盘的追问经宿主的路由到这里——`DashboardNavigation` 的 `unsaved` 原样放进来就是。每个**新对象**经离开守卫打开一次，与「从零做一个」同一条持有的路（没有 origin，按 id 开的那个随之放手），第一次保存即新建；同一个对象再交一次什么也不开（宿主带着同一条路由重渲染不该多出一份），种类不在 `kinds` 里的不开。打开是一个 effect：StrictMode 预演卸载时工作台照例放掉持有的视图，「见过」也一并忘掉，第二次运行重新打开。（见 test/dashboardPressUi.test.tsx「a view nobody saved, opened in the workbench」）
