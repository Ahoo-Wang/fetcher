...
两级 severity 在界面上分开呈现。`error` 阻塞：工作台以 destructive 变体的 `Alert` 报出（`label.view.needs-fixing`／`label.dashboard.needs-fixing`），`EmbeddedView` 以它取代结果（warning 仍照常显示在旁，两级并存时两级都说）。`warning` 不阻塞，但"报出而不阻塞"要求它被看见：`WarningNotice` 以 `role="status"` 和主题的 `warning` token（`text-warning`／`border-warning`，与 `destructive` 并列，宿主用 `--fve-warning`／`--fve-dark-warning` 定制）显示全部 warning，标题 `label.view.warnings`，不替换结果、不禁用任何按钮。三个工作台把它放在 error 提示之下、结果之上；`EmbeddedView` 同样显示——嵌入页没有编辑器，这是读者得知视图与作者所存不完全一致的唯一途径。Dashboard 只在这里显示自己的 warning（`useDashboard().issues`，不含 `['panels', …]` 路径），面板级的由面板自己呈现：不可用的面板在正文里说明理由，能运行却带 warning 的面板（子 runtime 对自身配置的 warning 已重定址到面板，见第 6 节）照常显示视图，头部加 `panel-warning` 标记（图标的可访问名与 `title` 是 warning 文案）并以 `data-warning` 标出边框。`FilterPanel` 的条件 pill 见下文。
