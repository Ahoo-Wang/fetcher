...
export interface ChartSpec {
  ...
  labels?: boolean; // 数据标签
  colors?: Record<string, string>; // 系列别名或分类值 → 颜色；未列出的用主题调色板；值须是 CSS 颜色（校验），级联系列先按 key 再按 metric 别名匹配
}
