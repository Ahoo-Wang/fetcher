...
afterEach(cleanup);
it('keeps old row scope and prevents row/toolbar writes while the new scope is waiting', async () => {
  ...
  const scopedDefinition = {
    ...
    record: {
      ...definition.record!,
      recordActions: { row: { name: 'guarded' }, toolbar: { name: 'guarded' } },
    },
  };
  ...
  engine.setPositionScope(position.identity.id, filter.eq('state.amount', 10));
  ...
  function View() {
    ...
    return session?.kind === 'record' ? (
      <RecordContent
        ...
        getSnapshot={() => {
          const current = engine.getSnapshot().sessions[position.identity.id];
