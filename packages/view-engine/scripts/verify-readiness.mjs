...
try {
  ...
  // They measure inside the page, so browser-driver transport is excluded from the 200/225ms budgets.
  report.analysis = {};
  await page.setViewportSize({ width: 1440, height: 1000 });
  for (const scenario of ['local-performance', 'large-result-cancellation']) {
    ...
    if (scenario === 'local-performance') {
      ...
      assert.ok(evidence.input.p95Ms <= 200, 'Input P95 exceeds 200ms');
      assert.ok(
        evidence.instanceSwitch.p95Ms <= 225,
        'Local switch P95 exceeds 225ms',
      );
