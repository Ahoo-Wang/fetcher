...
describe('accessor-safe endpoint binding', () => {
  it('does not evaluate own or inherited getters while decorating a class', async () => {
    class Base {
      state = { ready: true };
      get ready() {
    ...
    expect(() => api('/api')(Child)).not.toThrow();
    const child = new Child();
    expect(child.ready).toBe(true);
    expect(child.ownReady).toBe(true);
    await expect(child.list()).resolves.toEqual({ mocked: 'response' });
  });
});
