  ...
  vi.stubGlobal(
    ...
    async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (
        ...
        const query = JSON.parse(init?.body as string) as ListQuery;
        const matches = (
          ...
          if (condition.operator === Operator.AND)
            return (
              condition.children?.every(child => matches(snapshot, child)) ??
              true
            );
          const value = condition.field
            ?.split('.')
            .reduce<unknown>(
              (value, key) => Reflect.get(value as object, key),
              snapshot,
            );
        ...
        const response = () => {
          const available = server.hideCreatedView
            ? []
            : (server.snapshots ??
          ...
          const snapshots =
            query.limit === 1
              ? available
                  .filter(snapshot => matches(snapshot, query.condition))
                  .slice(0, 1)
              : available;
        ...
        if (server.deferLists && query.limit !== 1) {
