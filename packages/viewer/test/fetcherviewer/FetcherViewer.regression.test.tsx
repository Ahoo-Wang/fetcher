...
vi.mock('@ahoo-wang/fetcher-react', async original => {
  ...
  return {
    ...
    useQuery: () => {
      const [request, setRequest] = r.useState<any>();
      const setQuery = r.useCallback((request: any) => {
        state.queries.push(request.query);
        state.requests.push({ url: request.url, query: request.query });
        setRequest(request);
      }, []);
      const result = r.useMemo(
        () => (request ? { request, data: state.data } : undefined),
        [request, state.data],
      );
      return {
        result,
...
it('uses the selected view after initialization with defaultViewId', async () => {
  ...
  act(() => state.viewerProps.onLoadData(views[1].condition, 1, 10, []));
...
it.each(['tenantId', 'ownerId'] as const)(
  'isolates the selected view and rows when %s changes',
  async scope => {
    const pending = new Map<string, (rows: { id: string }[]) => void>();
    const enhance = (rows: { id: string }[]) =>
      new Promise<{ id: string }[]>(resolve =>
        pending.set(rows[0].id, resolve),
      );
    const { rerender } = render(
      <FetcherViewer
        viewerDefinitionId="def"
    ...
    fireEvent.click(screen.getByText('switch B'));
    await act(async () => pending.get('A')!([{ id: 'old-scope-row' }]));
    expect(screen.getByText(/old-scope-row/)).toBeTruthy();

    state.views = [
      {
        ...views[0],
        id: 'new-scope',
        internalCondition: { operator: 'EQ', field: 'scope', value: 'current' },
      },
    ];
    state.data = { list: [{ id: 'current' }], total: 1 };
    rerender(
      <FetcherViewer
        viewerDefinitionId="def"
        tenantId={scope === 'tenantId' ? 'tenant-b' : 'tenant-a'}
        ownerId={scope === 'ownerId' ? 'owner-b' : 'owner-a'}
        pagination={false}
        enhanceDataSource={enhance}
      />,
    );
    expect(screen.queryByText(/old-scope-row/)).toBeNull();
    expect(state.requests.at(-1).query.condition.children[0]).toEqual({
      operator: 'EQ',
      field: 'scope',
      value: 'current',
    });
    await act(async () => pending.get('current')!([{ id: 'current-row' }]));
    expect(screen.getByText(/current-row/)).toBeTruthy();
    expect(state.viewerProps.defaultView.id).toBe('new-scope');
  },
);
