...
核查7条待回复评论，6条成立并修复：定义权限异常统一降级为拒绝，覆盖创建能力和草稿编辑权限；引用身份/版本变化时重置面板渲染错误边界；重载保留无效编辑器缓冲及冲突；运行时元数据预算不足时保留页面导航并提供重试/重载；还原清除确定性写入错误；权限中断、保存/重载更新基线或还原改变编辑代次时结束旧布局编辑会话。

资源恢复另覆盖两个反例：重新加载期间导航离开后不得启动旧仪表盘；React编译模式不能仅靠未消费的刷新计数让缓存失效，改为保存实际恢复的运行对象。没有持久化加载服务时，也可在其他运行对象释放预算后重试。旧内容卡片和Storybook测试同步验证保存结束旧布局会话后重新进入编辑再移除，避免取消撤销已保存布局。

不采纳“所有引用均成功加载后才允许保存”的限制：配置结构、编辑器有效性、已注册转换器及当前可验证的绑定必须有效；加载中或不可访问引用保留在配置中，对应面板阻断，保存不授予访问权、不触发查询。补充中英文保存边界说明及行为反例：pending/FORBIDDEN引用仍可随有效配置保存，已加载元数据证明字段映射无效则拒绝保存。

新增16项恢复/权限/布局/保存边界用例。最终全仓串行单测通过，View Engine源码及React编译模式各1617通过、3跳过；374项Chrome Storybook交互与可访问性检查通过；受影响包及依赖构建、类型与扩展契约检查、ESLint、变更文件格式检查通过；11项文档检查及VitePress构建20.32s通过。覆盖率和超时门槛未调整。完整生产fixture、发布包一致性和真实业务服务未在本轮重跑；上节对应旧提交的验证不冒充本轮生产准入。

## 2026-09-13：重载预算、操作生命周期与有效性键

3条新增评论均成立。DashboardRuntime在接纳会话新配置前，计算协调后的applied与保留引用集合并预留完整额度；成功后才切换运行配置和释放移除引用。失败保留已接纳运行配置及权威重载会话，显示资源错误、禁止编辑，释放预算后可以重试。配置身份未变的还原仍清理临时有效性状态；无关通知继续先比较引用，避免反复深比较配置。

RecordContent业务操作守卫读取最近已提交props或实时读取器，按结果/作用域及选择代次判断，覆盖省略getSnapshot、卸载和旧结果A→B→A回显。刷新、列配置和分页等显式视图命令继续绑定原实例，保持既有引擎生命周期语义；既有RecordRegions用例及新增迟到业务操作反例分别验证这两个边界。

新增核心导出dashboardEditorKey(filterId, panelId?)，转换器ID使用JSON元组编码。React编辑器、运行时已知键验证和状态清理复用同一函数；API及中英文说明同步，打包消费者验证新增导出。使用(a:b,c)与(a,b:c)的实际编辑器用例验证一个编辑器变为有效不会清除另一个的无效状态。

新增7项回归；仪表盘套件146通过、1跳过，关键路径React编译模式44通过。最终全仓串行单测通过，View Engine源码及编译各1624通过、3跳过；374项Chrome Storybook交互与可访问性检查通过。包及依赖构建、类型检查、ESLint、格式、11项文档检查及文档构建18.72s通过。发布包验证459个dist文件逐字节一致、11个公开目标、2个核心运行模块；归档SHA256为12b4bda440b4f9d5ce548c36cbd7b2d37c2adbc729bdaeb824e21e2d4f10711b。完整生产性能fixture与真实业务服务未在本轮重跑。修订前e32e1e5d的远端CI已全部通过，新提交仍以其独立CI结果为准。

## 2026-09-13：长ID、布局资源失败与HTTP组合边界

本轮处理3条评论。修复生成有效性键的独立4096字符限制：合法长筛选/面板ID继续受配置总预算及已知编辑器身份校验约束，无效状态可以正常记录并阻止保存。

DashboardLayoutBoundary位于懒加载布局之外，拥有资源加载失败、简洁布局、重试和焦点恢复。失败期间保留导航、数据和配置草稿，仅暂停拖动编辑；重试使用新lazy资源，持续失败时仍可保存。面板消失后重新挂载健康布局会清除旧不可用标志。数据查询和运行位置仍由runtime持有，不因布局恢复重建。生产浏览器验证实际阻断DashboardGrid构建文件，确认可查看数据、新增Markdown、重复重试并保存成功，且没有逃逸的pageerror；该验证加入verify:view-engine标准流程。降级截图检查后补齐提示与面板间距。

HTTP评论确认了未发布实验客户端的单一定义限制，本轮不新增全局实验端点。补充按已授权目录选择目标HttpViewHost并组合公共ViewHost的双语示例及真实HTTP测试：纯仪表盘根定义可以查询另一个定义的记录面板；根保存只产生一次根服务写入，子服务无写入；子定义更高版本的授权不会覆盖根授权。原始单一定义客户端仍明确拒绝不属于本定义的资源；应用负责唯一实例ID、归属解析和访问范围变化时的客户端重建。

新增5项回归。最终全仓串行单测通过，View Engine源码及React编译模式各1629通过、3跳过；374项Chrome Storybook交互与可访问性测试通过；类型、ESLint、格式、包及依赖构建通过。完整本地Chromium verify:view-engine通过，覆盖打包消费者、HTTP/IndexedDB宿主、生产构建、布局资源失败及性能/可访问性fixture。461个dist文件逐字节一致，11个公开目标、2个核心运行模块；归档SHA256为b63717b7ef1975fbc175f65d35a94cb9668d4f90b222ea33305c1b38117ec2ae。11项文档检查及VitePress构建18.54s通过。真实业务服务、设备和业务用户走查未在本轮重跑，本地fixture通过不代表业务宿主生产准入。
