...
只有 `type: 'number'` 字段支持合计、平均值、最小值、最大值汇总；定义的 `summaryFunctions` 限制可用能力，实例列的 `summary` 保存选择，空数组可以关闭能力。其他类型和操作列不支持汇总，不提供 COUNT 记录数汇总。表格同时显示“本页”和“所有”两行，不再使用互斥范围配置或选择器。
...
数值运算忽略 null/缺失值，空集结果为 null 并显示“—”，真实零保留为零。本页非法数值和服务端缺失/非法结果独立报错。无分组聚合必须返回一行，每个指标使用稳定列 ID 顺序生成的别名，最多 64 项。两行拥有独立加载与错误状态，所有汇总失败保留本页结果和业务记录，提供独立重试；取消全部汇总列时终止在途聚合。分开的记录与汇总请求不承诺同一事务快照。没有可见汇总列时隐藏汇总行。
