...
布局采用紧凑业务工作台：标题与全局工具栏合并，左侧依次为标题、当前实例和保存组合按钮，菜单提供另存为与还原；无法覆盖原实例时另存为作为主按钮。全局创建、刷新组合按钮与页面展开位于右侧，筛选组合按钮提供展开/收起与简单/高级模式选择，无独立筛选标题行。筛选区添加条件在左，撤销、清空、查询在右；独立表格工具栏左侧显示勾选数量与取消选择，批量操作与列设置位于右侧。筛选区、表格与统计分页底栏组成连续的数据区域。筛选默认展开，可收起以增加记录空间，编辑器保持挂载；未查询草稿在展开时于查询附近提示，收起时由筛选按钮提示；当前标题和侧栏不重复提示，其他实例保留待查询标记。收起不改变查询、勾选或实例配置。筛选按钮显示已应用条件数量，Tooltip 保留逻辑分组与原阈值，待查询草稿不替换摘要。侧栏和页头使用更小间距，窄容器内工具栏与底栏自动换行。未固定、没有枚举选项的 string 列省略 width 时从 180px 起均分余宽，最高 480px；显式宽度和其他列维持原尺寸。数值默认右对齐并使用等宽数字。手动调宽后保存实际新宽度；列设置不提供宽度输入，仅通过表头边缘拖动设置，保留键盘调宽作为无障碍替代。自动适配不修改实例和查询，余白位于右固定区域之前。

固定区域令业务字段不足 128px 时，使用临时紧凑布局：主键缩窄、保留可辨认的尾部并可查看完整值，操作列使用 64px 图标弹层，普通固定列暂随中间区域滚动。宽度恢复后还原偏好；紧凑模式的主键/操作列自动定宽，在常规布局调整其宽度。极窄容器或强制固定列过多时提示空间不足。查询失败位于记录区域，区分于空数据，不显示零条记录或分页；后台失败保留上次结果。自动刷新暂停提供原因与恢复说明；普通保存成功短暂显示“已保存”并播报。
