...
日期：2026-09-05。状态：已落实用户确认的第一性原理审查修正；本文定义设计与验收契约，尚未实现。
...
运行结果、加载与保存状态、错误、行选中状态、页码、游标、筛选编辑模式及拖动过程数据不写入实例配置。内置及自定义筛选器都是 Wow 表达式编辑器，已提交的值必须能从 `FilterExpression` 无损恢复，不再持久化另一份筛选表单状态。渲染引用的 JSON `options` 描述控件设置，不承载另一份可独立修改的查询条件。
...
11. 自定义筛选器、数据单元格、操作单元格验证 props / 回调契约、缺失引用、非法输出、异常隔离、视图树作用域，以及扩展引用的 JSON 保存恢复。筛选器验证表达式无损往返、不兼容节点切回内置编辑器时保留参数；操作列无需数据字段，不应进入 Wow 字段排序。
...
本文记录已确认的设计与审查修正，不表示新 lib 已实现或已完成服务端运行时验证。实现计划须以本文的原样保存、固定访问范围、筛选语义转换和表达式无损编辑契约为依据。
