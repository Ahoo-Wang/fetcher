...
字段在添加筛选时选定。字段过滤器不提供更换字段的下拉，也不接受输出另一字段的 `onChange`；需要另一字段时添加对应过滤器。字段绑定来自现有表达式的 `field` 与当前作用域，不在实例 JSON 中重复存储。自定义字段筛选器遵循相同约束；同一字段可以绑定多个彼此独立的组件。
...
以平铺条件组成顶层 AND，不额外显示 AND 文字或条件间的“且”。同一字段可出现多次，例如金额大于等于一个值且小于另一个值。新增时从“添加筛选”选择字段，创建后固定显示字段名；操作列表来自这个字段的能力，默认操作选择常用且适合该字段的选项。
...
使用条件树编辑 AND、OR、NOR。组合选择器以中文分别表达“满足全部条件”“满足任一条件”“全部条件均不满足”，不重复附加 AND 等协议标记。面板内非容器条件保持统一组合，在同一分组中使用等宽网格，空间不足时自动换为单列；删除按钮右对齐。只有容器跨越网格并承担分组层级。独立使用的单字段组件仍按内容宽度排列。简单模式不提供条件操作菜单和前后排序，清空由值控件提供。高级模式只在存在可用目标分组时显示移动入口。可新增、删除和调整条件或分组，移动操作必须保留节点全部参数和绑定字段，并重新校验目标作用域；不能通过移动把根字段暗改为同名元素字段，键盘可以完成同样操作。
...
`packages/view-engine` 已独立承载 `FilterSelect`、固定字段 `FieldFilter`、`FilterDatePicker`、`FilterTimeInput` 和 shadcn 组合组件。操作、枚举值、添加字段和逻辑组合使用 Select；日期使用 Calendar / Popover，时间使用 InputGroup / Popover / Select。时间输入保留未完成文本与小数秒，宿主决定时区、精度和查询生效时机。菜单沿用默认 Neutral 主题、选中勾选和键盘交互，默认 Portal 到 body，并在打开时传递当前主题，避免被宿主容器裁剪。Storybook 的 `View Engine/Date and Time` 直接消费包的公开构建产物，演示日期、时间、手动查询、未完成输入和深浅主题。`FilterPanel` 已实现完整编辑缓冲、编译校验、条件树移动和扩展隔离；完整 ViewEngine 的实例运行状态仍由后续视图层承载。样式依据：[Input Group](https://ui.shadcn.com/docs/components/base/input-group)、[Select](https://ui.shadcn.com/docs/components/base/select)、[Theming](https://ui.shadcn.com/docs/theming)。
...
- 检查简单 AND、相同字段多个条件、嵌套逻辑和元素相对作用域，以及不能无损切回简单的反馈。
