...
字段在添加筛选时选定。字段过滤器不提供更换字段的下拉，也不接受输出另一字段的 `onChange`；需要另一字段时添加对应过滤器。字段绑定来自现有表达式的 `field` 与当前作用域，不在实例 JSON 中重复存储。自定义字段筛选器遵循相同绑定约束。简单模式每个字段只出现一次，未设置值也占用字段；高级模式的 AND、OR、NOR 及元素作用域均允许同一字段多条规则，编译和实例保存不禁止重复字段。包含重复字段的草稿不能切回简单模式。
...
“添加筛选”使用锚定按钮的 Popover 浮层，按字段定义的 `group` 分组展示 Checkbox；勾选添加字段条件，取消勾选移除当前组该字段的直接条件。简单模式不提供追加入口；高级模式显示字段的直接条件数量，并通过“追加条件”加号继续添加，AND、OR、NOR 交互一致。高级模式在“添加筛选”旁增加图标下拉按钮，提供 AND/OR/NOR 及中文说明，添加到当前分组；这三项不进入字段选择面板，并遵守定义中的操作白名单。浮层不占用页面布局空间，限制宽高并在字段区内部滚动。选择后保持打开，支持连续添加；“完成”或 Esc 关闭并返回触发按钮焦点，也支持点击外部关闭。
...
- 检查简单模式字段唯一性、高级 AND/OR/NOR 相同字段多条件、嵌套逻辑和元素相对作用域，以及不能无损切回简单的反馈。
