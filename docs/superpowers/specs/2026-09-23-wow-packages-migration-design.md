...
**时机**：view-engine 阶段 3 收口以后（批 D 完成，阶段审查与重构都已合并），阶段 4 开始之前。迁移窗口里连续做完第 0～3 步，阶段 4 起直接在 Wow 里开发，阶段 6 的存储后端也就原生地落在 Wow 里。阶段 4（嵌入视图）在迁移之前已经在 fetcher 合并（#1863，用户 2026-09-23 同意），于是在 Wow 里接着开发的是它之后的阶段。
