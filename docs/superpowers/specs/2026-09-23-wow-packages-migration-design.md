...
方案已与用户逐条确认（2026-09-23），经两轮自审，补充项均已并入。**尚未动手**；执行从「时机与步骤」开始。
...
`documentation` 部署时用的 `--shamefully-hoist`（#2170 为 mermaid 加的）不带进工作区。它会对全仓生效，把所有依赖提升到根目录，掩盖各个包漏声明的依赖。改用 `public-hoist-pattern`，只提升 mermaid 相关的包。
...
**时机**：view-engine 阶段 3 收口以后（批 D 完成，阶段审查与重构都已合并），阶段 4 开始之前。迁移窗口里连续做完第 0～3 步，阶段 4 起直接在 Wow 里开发，阶段 6 的存储后端也就原生地落在 Wow 里。
...
**开始前要满足三个条件**：`gh pr list` 为空；本地没有未推送的分支（包括三个故事场景分支 customer、trade-order、product-pricing）；worktree 都清理干净。
...
| 3′   | fetcher | 删掉已迁走的路径、viewer、dataMonitor、对应的故事和 `generator-test.yml`；suite 只剩 core；新增依赖方向检查和 `downstream-wow.yml`。合并前确认 `git diff wow-migration-base..HEAD -- <已迁走的路径>` 为空。**合并后解除冻结；main 在 6.0 之前不发布**，这期间 fetcher 的修复从 `5.x` 分支发补丁                                                                                                                                                    |
...
**带历史迁移**：在一个临时的 fetcher 克隆里跑 `git filter-repo`（本机还没装，需要先 `brew install git-filter-repo`），用 `--path-rename` 映射到 `typescript/…`，用 `--message-callback` 把提交信息里的 `(#1234)` 改写成 `(Ahoo-Wang/fetcher#1234)`，否则这些编号在 Wow 里会链接到别的 PR。**Wow 仓现在只允许 squash 和 rebase 合并**（`allow_merge_commit: false`）。squash 会把历史压成一个提交，rebase 要重放上千个提交。所以第 2、3 步的导入 PR 需要**临时开启 merge commit**，合并后再关掉。这是仓库设置，由用户操作。
...
| 任务                                                                                                                                                                                                                           | 仓库 / 会话    | 依赖                 | 能否并行                                                    |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------- | -------------------- | ----------------------------------------------------------- |
| P0 上下文：把 view-engine 相关记忆复制到 Wow 的项目记忆目录（`-Users-ahoo-work-ahoo-git-Wow`，合并进它已有的 `MEMORY.md`），fetcher 这边留指针；本文复制到 Wow 的 `docs/superpowers/specs/`，此后以 Wow 那份为准并在那里记进度 | fetcher 会话   | 阶段 3 收口          | —                                                           |
...
| T1 = 4a 的判据③：切换 `wow-project-template/client`                                                                                                                                                                            | 模板仓库的会话 | Wow 首个稳定版       | CoSky、PrajnaBot、ai 等其他下游项目是否一起切换，由用户决定 |
