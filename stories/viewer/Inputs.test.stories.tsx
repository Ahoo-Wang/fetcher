...
import displayMeta, {
  NumberRangeValidation as DisplayNumberRangeValidation,
  RemoteEmpty as DisplayRemoteEmpty,
  RemoteError as DisplayRemoteError,
  RemoteLoadingAndSuccess as DisplayRemoteLoadingAndSuccess,
  Tags as DisplayTags,
} from './Inputs.stories.js';
import type { StoryObj as RegressionStoryObj } from '@storybook/react-vite';

const meta = {
  ...displayMeta,
  title: 'Viewer/输入与过滤/Inputs/回归',
...
export const RemoteLoadingAndSuccess: Story = {
  ...DisplayRemoteLoadingAndSuccess,
  tags: ['!dev', '!autodocs', 'test'],
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    const input = canvas.getByRole('combobox', { name: 'User search' });
    ...
    await userEvent.click(
      canvas.getByRole('button', { name: 'Complete search' }),
    );
    await userEvent.click(input);
