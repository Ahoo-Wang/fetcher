...
2. Implement the applicable `ViewHost` services: `definition`, `instance`, `preference`, `permission`, plus `resolveSource` for business queries. The host provides metadata, persistence, current-user authorization and request identity/CAS guarantees: read ports take `{ readFence?, signal? }`, `instance.list` returns a paged `Page<ViewInstanceSummary>`, and every write takes `{ requestId, signal? }` and resolves to a `WriteObservation`. Reuse Fetcher/Wow query clients and existing storage APIs.
3. Let the engine coordinate selection, filter drafts/applied/saved state, paged/cursor navigation, queries, summaries, saving and recovery. Complete workbenches use engine commands for these operations. A local catalog is a `ViewInstance[]` with an optional `defaultInstanceId`; otherwise the engine pages the host catalog and reads the personal default from `preference.load`. Saved operators must satisfy both definition-wide and field-level restrictions.
