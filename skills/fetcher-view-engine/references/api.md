...
The browser/React entry (`@ahoo-wang/fetcher-view-engine/react`) additionally exports `IndexedDBViewHost` and `IndexedDBViewHostOptions`; the headless core remains free of browser globals. The class reuses LocalStorageViewHost methods, permissions and receipts. Options are `Omit<LocalStorageViewHostOptions, 'storage' | 'lock'>` plus optional `databaseName?: string` (default `fve-view-state`) and `legacyStorage?: Pick<LocalStorageViewHostOptions['storage'], 'getItem'>`. No storage or lock injection is required. Every operation uses an IndexedDB readwrite transaction; results resolve after commit, errors/abort roll back, and reset writes a null tombstone. Legacy snapshots import only when no database record exists; refresh all older tabs when migrating. Native localStorage + Web Locks must not be advertised as atomic cross-tab CAS. Keep LocalStorageViewHost for coherent synchronous injected service storage; browser persistence examples use IndexedDBViewHost.
