...
| Derived metric `props.displayFormat`               | `number` (default) or `percent`. Percent requires no known physical unit and uses Intl formatting without modifying the raw value.                                                   |
...
`AnalysisEditorProps.filterEditors` forwards definition-level filter editor mappings to metric and element panels. `appliedValue` supplies the last executed, same-scope metric filter by component ID for undo. Execution summaries describe metric predicates and HAVING from the retained query plan.
