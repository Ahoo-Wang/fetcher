...
- Conditional return type: literal `stream: true` yields `JsonServerSentEventStream<ChatResponse>`; literal `false`, `undefined`, or an omitted flag yields `ChatResponse`. A broad `boolean` flag or `ChatRequest` yields their union; narrow with `result instanceof ReadableStream` before consuming it.
