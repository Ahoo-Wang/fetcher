...
| `quality.yml`                    | CI policy tests (including the agent-skill drift guard, `skills.test.mjs`), the dependency-direction check (nothing may depend on `@ahoo-wang/wow-*`), changed-file formatting, read-only lint, all-package source type checks and documentation build.    |
